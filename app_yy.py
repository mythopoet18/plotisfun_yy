...
#    dp=requests.get('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=MSFT&apikey=demo')
    ...
    with open('ts_temp.json','r') as dp1:
        df=pd.read_json(dp1)
        t2=df.T
# Return to home if there is not enough dataframe
    if len(t2)<22:
        return redirect('index')
    else:
        ...
        ma0=tavg.rolling(lw).median()
        std0=tavg.rolling(lw).std()

        ma1=ma0.tail(len(t2)-lw+1).copy()
        ma1.index=t2.index[:len(t2)-lw+1]
        std1=std0.tail(len(t2)-lw+1).copy()
        std1.index=t2.index[:len(t2)-lw+1]

        blg_upper=ma1+std1*2
        blg_lower=ma1-std1*2

        t2['avg']=tavg
        t3=t2.head(len(t2)-lw+1).copy()
    ...
    # create plot 1
        ...
        #p.y_range = Range1d(t3['blg_lower'].min()/1.5, t3['blg_upper'].max()*1.5)
